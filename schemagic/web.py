...
def service_route(service, validation_pred=None, coerce_data=True, rule=None, input_schema=None, output_schema=None, fn=None):
    ...
    if fn is None:
        return partial(service_route, validation_pred, coerce_data, input_schema, output_schema)
    ...
    service.add_url_rule(
        ...
        view_func=update_wrapper(webservice_fn(fn, validation_pred, input_validator, output_validator), fn),
        methods=['POST']
    )
    return fn